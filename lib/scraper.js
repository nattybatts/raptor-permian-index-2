// Simple, direct Anthropic API search — matches the approach that worked on day 1
...
- "new ford f-150 raptor" odessa texas in stock
- "new ford raptor r" midland texas dealer
- rogers ford midland new raptor inventory
- sewell ford odessa raptor in stock
- new ford raptor for sale 79701

For every individual truck listing you find (new vehicles only, not used), record:
- VIN (17 characters, shown on every listing page)
- Trim: must be "Raptor" or "Raptor R" exactly — skip all other F-150 trims
- Model year
- Price/MSRP (exact dollar amount if shown, null if not listed)
- Dealer name
- Dealer city
- Direct URL to that specific vehicle's page on the dealer's site

Also find today's WTI crude oil price.

Respond with ONLY a JSON object, no markdown fences, no explanation text before or after:
{"wtiPrice":71.5,"vehicles":[{"vin":"1FTFW1RG0SFC00777","trim":"Raptor","model_year":2025,"model":"F-150","dealer_name":"Sewell Ford","dealer_city":"Odessa","dealer_state":"TX","msrp":104560,"vehicle_url":"https://www.sewellford.com/inventory/...","dealer_url":"https://www.sewellford.com"}],"commentary":"one sardonic sentence about what this truck count means for Permian Basin oilfield activity — low stock is good (boom), high stock is bad (bust)"}`;
  ...
  console.log('[scraper] raw text length:', raw.length);
  console.log('[scraper] raw preview:', raw.slice(0, 600));
  ...
  // Build dealer summary seeded with known dealers at 0
  const dm = new Map();
