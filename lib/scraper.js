...
If none found: {"vehicles":[]}`;
  ...
  try {
    ...
    // Filter out any Ranger Raptors — only accept F-150 Raptor/Raptor R
    // If model field is missing, accept it (prompt specified F-150 only)
    // If model field says Ranger, reject it
    const vehicles = (result.vehicles || [])
      .filter(v => !(v.model || '').toUpperCase().includes('RANGER'))
