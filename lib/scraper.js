...
export async function scrapeRaptors() {
  const params = new URLSearchParams({
    ...
    radius:   '100',
    rows:     '100',
    fields:   'vin,heading,price,msrp,exterior_color,interior_color,dom,dom_180,vdp_url,dealer,build,in_transit,vehicle_status,first_seen_at_date',
  });
  ...
  const vehicles = listings.map(v => ({
    ...
    model:        'F-150',
    trim:         (v.build?.engine_size && parseFloat(v.build.engine_size) >= 5.0) ? 'Raptor R' : 'Raptor',
    color:        v.exterior_color || null,
    msrp:         v.msrp || v.price || null,
    // Use first_seen_at_date for accurate days — dom can include pre-listing aggregator hits
    days_on_lot:     v.first_seen_at_date
      ? Math.floor((Date.now() - new Date(v.first_seen_at_date)) / 864e5)
      : (v.dom || 0),
    ...
    engine_size:     v.build?.engine_size ? parseFloat(v.build.engine_size) : null,
