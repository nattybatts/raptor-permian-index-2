...
// This guarantees every dealer gets checked, not cut off by a single search
async function searchDealer(dealer) {
  const prompt = `Search the web for new Ford F-150 Raptor and Raptor R trucks currently listed for sale at ${dealer.name} in ${dealer.city}, ${dealer.state}.

Their website is ${dealer.url}

Search for: new raptor inventory at ${dealer.name} ${dealer.city}
Also try visiting: ${dealer.inventoryUrl}

Return ONLY new vehicles (not used). Trim must say "Raptor" or "Raptor R" specifically.

If you find any Raptors, return this JSON with no other text:
{"vehicles":[{"vin":"<17-char VIN>","trim":"<Raptor or Raptor R>","model_year":<year>,"model":"F-150","msrp":<number or null>,"vehicle_url":"<direct URL to this specific truck listing>"}]}

If no Raptors found, return exactly: {"vehicles":[]}`;
  ...
  try {
    ...
    if (s < 0 || e < 0) {
      console.log(`[${dealer.name}] no JSON found`);
      return [];
    }
    ...
    console.log(`[${dealer.name}] found ${vehicles.length} Raptors`);
    return vehicles;
...
export async function fetchWTI() {
  // Alpha Vantage — same source as isoilabove100.com
  // Correct endpoint: function=WTI returns daily WTI crude spot price
  if (process.env.ALPHA_VANTAGE_KEY) {
    try {
      const url = `https://www.alphavantage.co/query?function=WTI&interval=daily&apikey=${process.env.ALPHA_VANTAGE_KEY}`;
      console.log('[wti] trying Alpha Vantage:', url.replace(process.env.ALPHA_VANTAGE_KEY, 'KEY'));
      const res = await fetch(url, { signal: AbortSignal.timeout(10000) });
      console.log('[wti] Alpha Vantage status:', res.status);
      if (res.ok) {
        const j = await res.json();
        console.log('[wti] Alpha Vantage response keys:', Object.keys(j));
        const data = j?.data || [];
        console.log('[wti] Alpha Vantage data length:', data.length, 'first:', data[0]);
    ...
    console.log('[wti] ALPHA_VANTAGE_KEY not set');
  }
...
export async function scrapeRaptors() {
  console.log(`[scraper] searching ${DEALERS.length} dealers in parallel...`);

  // Fire one search per dealer + WTI fetch, all at the same time
  ...
  // Collect all vehicles from all dealers
  const allVehicles = [];
  for (let i = 0; i < dealerResults.length; i++) {
    if (dealerResults[i].status === 'fulfilled') {
      allVehicles.push(...dealerResults[i].value);
    }
  }
  ...
  console.log(`[scraper] TOTAL: ${vehicles.length} vehicles (${raptor} Raptor, ${raptorR} Raptor R), WTI $${wti.price}`);
